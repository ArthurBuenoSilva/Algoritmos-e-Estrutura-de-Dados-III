...
void calcula_LPS(int lps[], char P[], int tamP)
{
	...
	{ 
        ...
        else // (p[i] != p[tam]) se encontrar uma letra que nao bate como prefixo e sufixo
        {
            ...
            { 
                lps[i] = 0; // o maior prefixo que tambem Ã© prefixo agora vale 0
                i++; // vou para o proximo prefixo
            } 
        } 
    }			
...
void kmp(char T[], char P[],int tamT, int tamP)
{
    ...
    while(i < tamT)
	{
        if(T[i] == P[j]) // match
		{
			j++;
			i++;
			if(j == tamP) // encontrei o padrao no texto na posicao (i-tamP) atÃ© i
			{
			   	cout << i-tamP << endl;
			   	j = lps[j-1];
		   	}
...
int main()
{
    char t[50];     // Texto a ser utilizado para a busca
    char p[50];     // Padrão
